...
int main(int argc, const char* argv[])
{
  ...
  UnitProblem<double> * unit_problem = new UnitProblem<double>();

  ReinforcementLearning * reinforcement = new ReinforcementLearning(unit_problem, "rl.txt");
    ...
    while(Broodwar->isInGame())
    {
	  ...
	  if (Broodwar->getFrameCount() % Broodwar->getLatencyFrames() == 0){
		  if (enemy_unit_discovered){
			  if (unit_problem){
				  reinforcement->run();
				  if (unit_problem->endOfEpisode()){
					  reinforcement->end("reinforcementLearning.bin");
				  }
			  }
			  else{
			  ...
			  Position p = unit_experiment->getPosition()+Position(64, 0);
